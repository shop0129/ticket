# V7.8.3.3 Sprint 6 FIX1

- 修正點餐機仍執行舊 PWA 時，按「現金付款」直接顯示付款成功、沒有進入配對碼流程。
- 新 Service Worker 安裝後立即接管，安全狀態下自動重載新版首頁。
- 刪除保留舊 PWA 快取的例外，部署後會清理所有同系列舊快取。
- 現金付款按鈕新增回歸測試，確認只啟動本機橋接，不可直接建立成功訂單。
- 合併右下角重複版本標籤，只保留 `V7.8.3.3 S6 FIX1 · ONLINE/OFFLINE`。
