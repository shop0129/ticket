...
function applyPaymentSetting() {
    ...
    if (lineBtn) {
        applyLinePayButtonSetting(lineBtn);
    }
    if (cartCash) {
        cartCash.disabled =
            !systemData.payment.cash;
    ...
    if (cartLine) {
        applyLinePayButtonSetting(cartLine);
    }
}

// FIX26E：LINE Pay 設定關閉時仍保留按鍵事件，點擊後才能顯示明確提示。
// 真正付款中仍使用原生 disabled，避免客人重複送出交易。
function applyLinePayButtonSetting(button) {
    var enabled = !!(systemData && systemData.payment && systemData.payment.linepay);
    var transactionLocked =
        (typeof paymentInProgress !== "undefined" && !!paymentInProgress) ||
        !!(
    ...
    button.disabled = transactionLocked;
    button.setAttribute("aria-disabled", enabled ? "false" : "true");
    button.setAttribute("data-payment-setting-disabled", enabled ? "0" : "1");
    button.title = enabled ? "" : "LINE Pay 已在系統設定中關閉";
}
