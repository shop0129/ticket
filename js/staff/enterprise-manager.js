...
function sortedTicketIds(){return Object.keys(tickets||{}).sort(function(a,b){var ao=Number(tickets[a]&&tickets[a].sortOrder),bo=Number(tickets[b]&&tickets[b].sortOrder);if(!isFinite(ao))ao=999999;if(!isFinite(bo))bo=999999;if(ao!==bo)return ao-bo;return String(a).localeCompare(String(b));});}
function normalizeTicketOrder(){sortedTicketIds().forEach(function(id,index){tickets[id].sortOrder=index;});}
function ticketArray(){return sortedTicketIds().map(function(id){return {id:id,data:tickets[id]||{}};});}
...
function ticketCard(x){var t=x.data,id=x.id;return '<article class="enterprise-ticket" data-id="'+esc(id)+'"><div class="ticket-card-top"><img src="images/'+esc(t.image||'ticket-bg.png')+'" onerror="this.src=\'images/ticket-bg.png\'"><div><input class="ticket-title" data-f="title" value="'+esc(t.title||'未命名票券')+'"><small>'+esc(id)+'</small></div><label class="switch"><input type="checkbox" data-f="enable" '+(t.enable!==false?'checked':'')+'><span></span></label></div><div class="ticket-fields"><label>價格<input type="number" min="0" data-f="price" value="'+Number(t.price||0)+'"></label><label>代幣<input type="number" min="0" data-f="token" value="'+Number(t.token||0)+'"></label><label>分類<select data-f="category"><option value="general" '+(t.category==='general'?'selected':'')+'>一般票</option><option value="special" '+(t.category==='special'?'selected':'')+'>限定票</option><option value="other" '+(!t.category||t.category==='other'?'selected':'')+'>其他票</option></select></label><label>玩具<select data-f="toy"><option value="none" '+(!t.toy||t.toy==='none'?'selected':'')+'>無</option><option value="green" '+(t.toy==='green'?'selected':'')+'>綠標</option><option value="red" '+(t.toy==='red'?'selected':'')+'>紅標</option></select></label><label>時間模式<select data-f="timeMode"><option value="duration" '+((t.timeMode||'duration')==='duration'?'selected':'')+'>⏱ 固定時數</option><option value="fixed" '+(t.timeMode==='fixed'?'selected':'')+'>🕒 到指定時間</option><option value="unlimited" '+(t.timeMode==='unlimited'?'selected':'')+'>∞ 無限時</option><option value="none" '+(t.timeMode==='none'?'selected':'')+'>— 不設定</option></select></label><label>固定時數<input type="number" min="1" max="99" data-f="playHours" value="'+Math.min(99,Math.max(1,Number(t.playHours||t.hour||2)))+'"></label><label>指定時間<input type="time" data-f="fixedExitTime" value="'+esc(t.fixedExitTime||'18:00')+'"></label><label class="wide">圖片檔名<input data-f="image" value="'+esc(t.image||'')+'"></label><label class="wide">說明<textarea data-f="description">'+esc(t.description||'')+'</textarea></label></div><div class="ticket-actions"><button data-act="up">⬆ 上移</button><button data-act="down">⬇ 下移</button><button data-act="bottom">⏬ 移至最下面</button><button data-act="copy">📄 複製</button><button data-act="delete" class="danger">🗑 刪除</button></div></article>';}
function collectTickets(){document.querySelectorAll('.enterprise-ticket').forEach(function(card){var id=card.dataset.id,t=tickets[id]||{};card.querySelectorAll('[data-f]').forEach(function(f){var k=f.dataset.f;if(k==='enable')t[k]=f.checked;else if(k==='price'||k==='token')t[k]=Math.max(0,Number(f.value||0));else if(k==='playHours')t[k]=Math.min(99,Math.max(1,Number(f.value||2)));else t[k]=String(f.value||'').trim();});tickets[id]=t;});}
function addTicket(){collectTickets();normalizeTicketOrder();var id='custom_'+Date.now();tickets[id]={title:'新票券',price:0,token:0,toy:'none',category:'other',enable:true,image:'ticket-bg.png',description:'',custom:true,canEnter:false,admissionRequired:false,timeMode:'none',playHours:2,playMinutes:0,fixedExitTime:'',sortOrder:sortedTicketIds().length};renderTickets();setTimeout(function(){var c=document.querySelector('[data-id="'+id+'"]');if(c)c.scrollIntoView({behavior:'smooth'});},30);}
function ticketAction(e){var btn=e.target.closest('[data-act]');if(!btn)return;collectTickets();normalizeTicketOrder();var card=btn.closest('.enterprise-ticket'),id=card.dataset.id,act=btn.dataset.act,ids=sortedTicketIds(),i=ids.indexOf(id);if(act==='delete'){if(!confirm('確定刪除「'+(tickets[id].title||id)+'」？'))return;delete tickets[id];normalizeTicketOrder();}else if(act==='copy'){var nid='custom_'+Date.now();tickets[nid]=JSON.parse(JSON.stringify(tickets[id]));tickets[nid].title=(tickets[id].title||'票券')+' 複製';tickets[nid].custom=true;tickets[nid].sortOrder=ids.length;}else if(act==='bottom'){if(i<0||i===ids.length-1)return;ids.splice(i,1);ids.push(id);ids.forEach(function(k,order){tickets[k].sortOrder=order;});}else{var j=i+(act==='up'?-1:1);if(j<0||j>=ids.length)return;var tmp=ids[i];ids[i]=ids[j];ids[j]=tmp;ids.forEach(function(k,order){tickets[k].sortOrder=order;});}renderTickets();}
...
function saveTickets(){collectTickets();normalizeTicketOrder();if(window.MonsterTicketDataSync)tickets=MonsterTicketDataSync.normalizeMap(tickets);var titles={};for(var id in tickets){var title=(tickets[id].title||'').trim();if(!title){notify('票券名稱不可空白',false);return;}if(titles[title]){notify('票券名稱重複：'+title,false);return;}titles[title]=true;var tm=tickets[id].timeMode||'duration';tickets[id].playHours=Math.min(99,Math.max(1,Number(tickets[id].playHours||2)));tickets[id].playMinutes=tm==='duration'?tickets[id].playHours*60:0;tickets[id].hour=tm==='duration'?tickets[id].playHours:0;tickets[id].fixedExitTime=tm==='fixed'?(tickets[id].fixedExitTime||'18:00'):'';if(tickets[id].canEnter===false)tickets[id].timeMode='none';tickets[id].reward=[Number(tickets[id].token)>0?'token':'',tickets[id].toy&&tickets[id].toy!=='none'?'toy':'',tickets[id].canEnter!==false?'band':'',tickets[id].pickupItem==='socks'?'socks':''].filter(Boolean).join(',');}
