...
var build = readProject(
    "02_Android_Kiosk118_Manual_Start_Only/webkiosk/build.gradle.kts"
);
var installer = readProject("01_INSTALL_KIOSK118_MANUAL_START_AND_REBOOT.cmd");
...
assert.ok(page.indexOf('version: "fix18"') >= 0);
...
assert.ok(activity.indexOf("markNativeHomeReady('kiosk118')") >= 0);
assert.ok(activity.indexOf("NATIVE_HOME_RELEASE_GUARD_MS = 700L") >= 0);
assert.ok(build.indexOf("versionCode = 118") >= 0);
assert.ok(build.indexOf("1.16-sprint11s-kiosk118-manual-start-only") >= 0);
assert.ok(installer.indexOf("versionCode=113") >= 0);
assert.ok(installer.indexOf("versionCode=118") >= 0);
